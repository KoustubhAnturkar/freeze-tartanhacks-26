...
class Game {
  constructor() {
    ...
    if (scoreIconEl && scoreIconEl.getContext) {
      try {
        ...
        this.scoreIconRenderer = new Renderer(
          iconCtx,
          scoreIconEl.width,
          scoreIconEl.height,
        );
  ...
  update() {
    ...
    if (
      this.gameState.isTutorialActive() ||
    ...
    if (this.gameState.checkIcicleCollisionWithPlatform(this.player)) {
      if (
        typeof SOUNDS !== "undefined" &&
        SOUNDS &&
        typeof SOUNDS.play === "function"
    ...
    if (
      this.gameState.checkGoalCollision(this.player) &&
      this.gameState.getCollectibles().length === 0
    ) {
      if (
        typeof SOUNDS !== "undefined" &&
        SOUNDS &&
